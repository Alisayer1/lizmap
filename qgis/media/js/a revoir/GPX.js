...
function initGpxView(activateGpxOnStartup) {
    var map, vectors, controls, Parcelle_Nice;
    var gpxLayer;
    ...
    function addLayerFile(){
        ...
        var result;
        var Parcelle_Nice = $('#gpx_file')[0].files[0].name;
        var ext = Parcelle_Nice.split('.')[1];
        Parcelle_Nice = Parcelle_Nice.split('.')[0];
    ...
    function export_file() {
        Parcelle_Nice = prompt("Entrez le nom du fichier (avec extension .gpx, .geojson ou .kml)", Parcelle_Nice);
        if(Parcelle_Nice != null && Parcelle_Nice != ""){
            ...
            var pieces = Parcelle_Nice.split('.');
            ...
            element.setAttribute('download', Parcelle_Nice);
